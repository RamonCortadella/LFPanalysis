close('all')
Par =  LoadXml('../DatData/Interpolated-NotFilteredLFP-HP-1HzAC-0p005HzDC/NotClippedMap/B13289O14-DH1-Rec9AC.xml');

d = dir('../DatData/Interpolated-NotFilteredLFP-HP-1HzAC-0p005HzDC/NotClippedMap/B13289O14-DH1-Rec9*.dat');%DC-LP30Hz-Notch50-100Hz.dat');
...
figure(1)
subplot(2,2,1)
[ppxAC, fAC] = pwelch(LfpGeom,2^14,2^10,2^14,Fs);
loglog(fAC,ppxAC)
title('AC-channels')
subplot(2,2,2)
[ppx, fAC] = pwelch(wx,2^14,2^10,2^14,Fs);
loglog(fAC,ppx)
title('whitened AC-channels')
subplot(2,2,3)
[ppxDC, fDC] = pwelch(LfpGeomDC(100*Fs:end,:),2^14,2^10,2^14,Fs);
loglog(fDC,ppxDC)
title('DC-channels')
subplot(2,2,4)
[ppx, fDC] = pwelch(wxDC(100*Fs:end,:),2^14,2^10,2^14,Fs);
loglog(fDC,ppx)
...
save(strcat('../MatlabData/',SplitName{3}(1:end-6),'-SpecHVS.mat'),'SpecHVS')
...
save(strcat('../MatlabData/',SplitName{3}(1:end-6),'-SpecStates.mat'),'SpecStates')
...
save(strcat('../MatlabData/',SplitName{3}(1:end-6),'-Traces.mat'),'LFP_traces')
...
plot((Sinit+(1:1:length(LfpGeomDC(Sinit:Sfin,1))))/LFPfs, LfpGeomDC(Sinit:Sfin,ChIndHVS))
