...
%         
    ...
    case 'groupstats'
                    ...
                    if strcmp(fn{i},StatesCell{i})
                        ...
                        if szTemp(1)>=1
                            for ii =1:length(PerStateTemp(:,1))
                                ...
                                c = [c,i];
                                DurationState = [DurationState,PerStateTemp(ii,2)-PerStateTemp(ii,1)];
                            end
                        end
                    end
                else
        ...
        if strcmp(TargetStateLabel,'all')
            ...
            figure()
            MarkerSize= 10;
            scatter3(MeanPowerDeltaArr(IndicesLong),MeanPowerThetaArr(IndicesLong),MeanPowerSpindleArr(IndicesLong),MarkerSize,c(IndicesLong),'filled')
            xlabel('PowerDelta')
            ylabel('PowerTheta')
            zlabel('PowerSpindle')
            xlim(prctile(MeanPowerDeltaArr(IndicesLong),[1 90]))
            ylim(prctile(MeanPowerThetaArr(IndicesLong),[1 90]))
            zlim(prctile(MeanPowerSpindleArr(IndicesLong),[1 90]))
            a= colorbar;
            ylabel(a,'duration')
            title(TargetState)
            ...
            figure()
            MarkerSize= 10;
            scatter3(MeanPowerThetaArr(IndicesLong)./MeanPowerDeltaArr(IndicesLong),MeanPowerSpindleArr(IndicesLong),DurationState(IndicesLong),MarkerSize,c(IndicesLong),'filled')
            xlabel('PowerTheta/Delta')
            ylabel('PowerSpindle')
            zlabel('Duration')
            xlim(prctile(MeanPowerThetaArr(IndicesLong)./MeanPowerDeltaArr(IndicesLong),[1 95]))
            ylim(prctile(MeanPowerSpindleArr(IndicesLong),[1 90]))
            zlim(prctile(DurationState(IndicesLong),[1 90]))
            a= colorbar;
            ylabel(a,'duration')
            title(TargetState)
            ...
            DurationState = PerState(:,2)-PerState(:,1);
            IndicesLong = find(DurationState>=DurationTh);
            DurationAboveThresh = DurationState(IndicesLong);
            ...
            ylabel(a,'duration')
            title(TargetState)
            display(size(PerState))

            figure()
            histogram(PerState(:,2)-PerState(:,1),100)
            xlabel('duration')
            ylabel('counts')
            title(TargetState)            
        end
