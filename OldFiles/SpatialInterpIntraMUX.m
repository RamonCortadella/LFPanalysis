...
Directories = ["../../../data/ASICintraMUX/rat01606-B13907O23-M5/DatData/2022.06.15-16.26.24-Rec3Cal.dat";];
% a(0)  
...
%     Par =  LoadXml(['../../../data/LargeScale/B13289O14-DH1-01463/Day1-09_10-12-21/DatData/ClippedMapped/B13289O14-DH1-Rec3' 'AC.xml']);
    Par =  LoadXml('../../../data/ASICintraMUX/rat01606-B13907O23-M5/DatData/2022.06.15-16.26.24-Rec3Cal.dat.xml');
...
%             LfpGeomDC(:,ii+(i-1)*16) = DCLfp(:,Par.AnatGrps(i).Channels(ii)+1);
    ...
    for k=1:numel(fn)
        ...
        for i = 1:length(Lfp(1,:))
            display(i,' channel')
            if mod(i,nChCol) <= 1
                Interp = logical(ones([length(LfpGeomClip2(:,1)),1]))*0;
            else
                if max(LfpGeomClip(:,i-1))~=1
                    if max(LfpGeomClip(:,i+1))~=1
                        
                        Interp = (Lfp(:,i-1)+Lfp(:,i+1))/2;
                    else
                        Interp = logical(ones([length(LfpGeomClip2(:,1)),1]))*0;
                    end
                    Interp = logical(ones([length(LfpGeomClip2(:,1)),1]))*0;
...
%         LfpStruct.(fn{k}) = Lfp;
    ...
    %%
    SaveBinary(strcat(Directory,'interp.lfp'), Lfp)
