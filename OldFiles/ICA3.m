...
[fMode, NumICs,BrainState,val,FreqBands,MinDurationStates,TimeScaleGlue,ThRoughness,interactiveDisplay,PhAmpKlustersAnimal] = DefaultArgs(varargin,{'compute',30,[],[],40,0.2,{'isa','delta'},1,'off',3}); %'isa','delta','theta','spindle','beta','lowgamma','highgamma'
display(fMode)
display(NumICs)
...
%           
    ...
    case 'PhAmpCoupling'
        ...
        if length(fieldnames(out)) >=2
            for IndLoopF = 1:length(fieldnames(out))-1
                for iFreq = 1:length(fieldnames(out))-IndLoopF
                    display(iFreq,'frequency index Ph-Amp')
                    display(IndLoopF,'Inter frequency step')
                    for i = 1:length(out.(FreqBands{iFreq}).ICAsig(:,1))
                        display(i,'IC index of low freqs')
                        
                        for i2 = 1:length(out.(FreqBands{iFreq+IndLoopF}).ICAsig(:,1))
% --------------------- implementation 1
...
% --------------------- implementation 2
                            FNi = Fs/2;
                            fPow = [out.(FreqBands{iFreq+IndLoopF}).Freq/FNi; (out.(FreqBands{iFreq+IndLoopF}).Freq+1)/FNi]; %the frequency band in the second position is only provided to temporarily avoid a bug//it is not used
                            fPh = [out.(FreqBands{iFreq}).Freq/FNi; (out.(FreqBands{iFreq}).Freq+1)/FNi]; 
