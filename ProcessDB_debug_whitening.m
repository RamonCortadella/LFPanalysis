...
BrainState='PerSWS';
FreqBandLow={'delta'};
FreqBandsHigh={'spindle','beta','lowgamma','highgamma'};
HighFreqCoupling='.AC';
FMinSpec =100;
DownFact=[];
Twindow=1;
...
%     display(LfpState(1:4,1:4))
    [wx,~] = WhitenSignal(LfpState(:,iCh),[],[],[],1);
    [s,f,t] = mtcsglong(wx,nFFT,FsHigh,nFFT,7*nFFT/8);%,1024);
    ...
    if length(Channels)~=nCh
        for iCh = Channels
            ...
            for iFreq  = 1:length(FreqBandsHigh)
                PowPeaks.(FreqBandsHigh{iFreq}) = pow.(FreqBandsHigh{iFreq})(floor(ICt(locs)*FsSpec),iCh);
        ...
        if Chs
            for DeltaT = [-0.1,0,0.1]
                for iFreq  = 1:length(FreqBandsHigh)
                    ...
                    hold on
                    for iCh = Chs
                        PowPeaksHist.(FreqBandsHigh{iFreq}) = pow.(FreqBandsHigh{iFreq})(floor((ICt(locs)+DeltaT)*FsSpec),iCh);            
...
%         
        %video of AvLfp
        OutputVideo = [OutputPath FreqBandLow{1} '-AvIC' num2str(iSelectICs) '-AvLfp' Coupling 'PeaksSign(' num2str(SignCorr) ')-spec.avi'];
        Video_powerbands(out.ICt,out.AvIC',LfpT,AvLfp, OutputVideo, T, indDB)        
        %video of power single event
        OutputVideo = [OutputPath FreqBandLow{1} '-AvIC' num2str(iSelectICs) '-PowerSingleEvent' HighFreqCoupling 'PeaksSign(' num2str(SignCorr) ')-spec.avi'];
        Video_powerbands(out.ICt,ICEvent,out.PowerT,PowEvent, OutputVideo, T, indDB)
        %video of Lfp single event
        OutputVideo = [OutputPath FreqBandLow{1} '-AvIC' num2str(iSelectICs) '-LfpSingleEvent' HighFreqCoupling 'PeaksSign(' num2str(SignCorr) ')-spec.avi'];
        ...
        for iCh = Channels
            ...
            imagesc(out.PowerT,AvSpec.f(indFreqSpec),log10(whitened(indFreqSpec,:)))
            ...
            for iFreq = 1:length(FreqBandsHigh)
                ...
                plot(out.PowerT,PowTrace,'DisplayName',FreqBandsHigh{iFreq})%/max(abs(PowTrace))
            end
            ...
            imagesc(out.PowerT,AvSpec.f(indFreqSpec),log10(whitenedEvent(indFreqSpec,:)))
